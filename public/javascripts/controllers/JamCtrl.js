angular.module('jamSpaces')
	.controller('JamCtrl', ['$scope', '$http','$routeParams', '$location', '$rootScope', function($scope, $http, $routeParams, $location, $rootScope){
...
// the changing drum kit logic
	...
	$scope.currentKit = $scope.samplesets[x].kit

	$scope.changeKitUp = function (){
		(x++)
		$scope.currentKit = $scope.samplesets[x].kit
		hat = new Wad({source : '/samples/' + $scope.currentKit + '/1.wav'})
		kick = new Wad({source : '/samples/' + $scope.currentKit + '/2.wav'})
		snare = new Wad({source : '/samples/' + $scope.currentKit + '/3.wav'})
		aux = new Wad({source : '/samples/' + $scope.currentKit + '/4.wav'})
		pad5 = new Wad({source : '/samples/' + $scope.currentKit + '/5.wav'})
		pad6 = new Wad({source : '/samples/' + $scope.currentKit + '/6.wav'})
		pad7 = new Wad({source : '/samples/' + $scope.currentKit + '/7.wav'})
		pad8 = new Wad({source : '/samples/' + $scope.currentKit + '/8.wav'})
	}
...
//globally defining the pads ------------------------------------------
	var hat = new Wad({source : '/samples/' + $scope.currentKit + '/1.wav'})
	var kick = new Wad({source : '/samples/' + $scope.currentKit + '/2.wav'})
	var snare = new Wad({source : '/samples/' + $scope.currentKit + '/3.wav'})
	var aux = new Wad({source : '/samples/' + $scope.currentKit + '/4.wav'})
	var pad5 = new Wad({source : '/samples/' + $scope.currentKit + '/5.wav'})
	var pad6 = new Wad({source : '/samples/' + $scope.currentKit + '/6.wav'})
	var pad7 = new Wad({source : '/samples/' + $scope.currentKit + '/7.wav'})
	var pad8 = new Wad({source : '/samples/' + $scope.currentKit + '/8.wav'})

//handling incoming drum machine "notes"------------------------------------
		socket.on('keyPressEmission', function(data){
			if (data.notes === 116){
				var ehat = new Wad({source : '/samples/' + data.drum + '/1.wav'})
				ehat.play()
			}
			if (data.notes === 114){
				var ekick = new Wad({source : '/samples/' + data.drum + '/2.wav'})
				ekick.play()
			}
			if (data.notes === 121){
				var esnare = new Wad({source : '/samples/' + data.drum + '/3.wav'})
				esnare.play()
			}

			if (data.notes === 117){
				var eaux = new Wad({source : '/samples/' + data.drum + '/4.wav'})
				eaux.play()
			}
			if (data.notes === 102){
				var epad5 = new Wad({source : '/samples/' + data.drum + '/5.wav'})
				epad5.play()
			}
			if (data.notes === 103){
				var epad6 = new Wad({source : '/samples/' + data.drum + '/6.wav'})
				epad6.play()
			}
			if (data.notes === 104){
				var epad7 = new Wad({source : '/samples/' + data.drum+ '/7.wav'})
				epad7.play()
			}
			if (data.notes === 106){
				var epad8 = new Wad({source : '/samples/' + data.drum + '/8.wav'})
				epad8.play()
			}
		})
...
//these are coming in off of the server on clicks and smartphone touches
		socket.on('touchedNotes', function(data){
			...
			if (data.note === 6){
				var epad6 = new new Wad({source : '/samples/' + data.drum + '/6.wav'})
				epad6.play()
			}
			if (data.note === 7){
				var epad7 = new Wad({source : '/samples/' + data.drum + '/7.wav'})
